...
def donate_message(app, message):
    message.reply_text(f"**Hello User, It's Our Privilege to respond to your Big Heart, if you're Wishing To Donate Us Some Penny. Click Below.** ```After Donating DM your Screenshot``` **@HeimanSSProof_bot** ", quote=True)

@app.on_message(filters.user(sudo_users) & filters.incoming & (filters.video | filters.document))
def encode_video(app, message):
    if message.document:
        if not message.document.mime_type in video_mimetype:
            message.reply_text("⚠️ ```Invalid Video!\nMake sure it's a valid video file.```", quote=True)
            return
    if message.text:
